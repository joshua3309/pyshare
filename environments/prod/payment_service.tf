module "payment_task" {
  ...
  family         = "${var.env}-${var.project}-payment"
  ...
  environment_variables = local.payment_env
...
module "payment_service" {
  ...
  service_name = "${var.env}-payment"

  cluster_id   = module.ecs_cluster.cluster_id
  cluster_name = module.ecs_cluster.cluster_name

  task_definition_arn = module.payment_task.task_definition_arn

  desired_count = var.desired_counts["payment"]
  ...
  service_connect_dns_name = "payment"

  service_connect_discovery_name = "payment"

  service_connect_port = 4003
