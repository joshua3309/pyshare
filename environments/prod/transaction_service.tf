module "transaction_task" {
  ...
  family         = "${var.env}-${var.project}-transaction"
  container_name = "transaction"
  image = var.transaction_image
  ...
  environment_variables = local.transaction_env
...
module "transaction_service" {
  ...
  service_name = "${var.env}-transaction"

  cluster_id   = module.ecs_cluster.cluster_id
  cluster_name = module.ecs_cluster.cluster_name

  task_definition_arn = module.transaction_task.task_definition_arn
  ...
  target_group_arn = module.alb.target_groups.transaction.arn
  ...
  service_connect_dns_name = "transaction"

  service_connect_discovery_name = "transaction"

  service_connect_port = 4004
